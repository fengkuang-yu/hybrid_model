...
pro_data = data_pro(data, time_steps=time_step)
smooth_data = np.zeros(shape=data.shape)
for i in range(len(pro_data)):
    seg_filted = seg_filter(pro_data[i], frac_param=0.3)
    smooth_data[time_step+i, -1] = seg_filted[-1, -1]
plt.plot(data)
plt.plot(smooth_data)
# 使用整体的平滑结果
filtered_data = lowess(data.flatten(),
                       np.array([x for x in range(len(data.flatten()))]),
                       is_sorted=True, frac=0.05, it=0)
